...
const QuestionCard: React.FC<QuestionCardProps> = ({ question }) => {
  ...
  // Check if user already answered this question in this session (optional, but good for UX)
  // For simplicity, we allow re-answering but only the first one counts for stats in a real app.
  // Here we'll implement it so answering updates the global stats.
  ...
  const handleAnswer = async (index: number) => {
    ...
    try {
      ...
      // Only update stats if it's the first time answering this specific question
      if (!responseDoc.exists()) {
        ...
        await updateDoc(userRef, {
          ...
          streak: newStreak,
          points: increment(isCorrect ? 10 : 2) // Bonus for gamification
        });
      }
    } catch (err) {
  ...
  return (
    <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-6 mb-6 hover:border-neutral-700 transition-all duration-300">
